# 030 Score Log — GH-300

## 문제은행 (Question Bank, QB)

| Date | Attempt | Correct | Total | Score | Weak Skill Area | Next Action |
|---|---:|---:|---:|---:|---|---|
|  | 1 |  | 100 |  |  |  |
|  | 2 |  | 100 |  |  |  |

## 모의고사 (Mock Exams, ME)

| Date | Mock | Correct | Total | Score | Weak Skill Area | Gate |
|---|---|---:|---:|---:|---|---|
|  | Mock 01 |  | 40 |  |  |  |
|  | Mock 02 |  | 40 |  |  |  |
|  | Final |  | 40 |  |  |  |

## Wrong 정답 Retry (Wrong Answer Retry, WAR)

| Date | Queue | Correct | Total | Score | Result |
|---|---|---:|---:|---:|---|
|  | +1 day |  |  |  |  |
|  | +7 day |  |  |  |  |

## Skill 영역 Snapshot (Skill Area Snapshot, SAS)

자가평가 또는 QBank/Mock 결과를 합산해 기록합니다.

| Skill Area | Confidence 1–5 | Latest Score | Note |
|---|---:|---:|---|
| Responsible AI |  |  |  |
| Copilot Features |  |  |  |
| Data / Architecture |  |  |  |
| Prompt / Context |  |  |  |
| Developer Productivity |  |  |  |
| Privacy / Safeguards |  |  |  |

## Trend

```text
Attempt 1:
Attempt 2:
Mock 01:
Mock 02:
Final:

Trend: UP / FLAT / DOWN
Strongest:
Weakest:
```

## Exam-Ready 기록

```text
Date:
QBank >= 85%: YES / NO
Recent two mocks >= 85%: YES / NO
Final >= 90%: YES / NO / NOT REQUIRED
Wrong-answer retry >= 90%: YES / NO
Official scope checked: YES / NO

Decision: EXAM-READY / REVIEW
```
